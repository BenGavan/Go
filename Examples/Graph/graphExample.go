...
func createNode(con bolt.Conn) error {
	queryString := "CREATE (n:USER {uuid: {uuid}, email: {email}})"
	st := prepareSatement(queryString, con)
	defer st.Close()
	result, err := st.ExecNeo(map[string]interface{}{"uuid": 2, "email": "ben@gmail.com"})
	if err != nil {
		return err
	}
	...
	fmt.Printf("NUUMBER OF rows affected/created = %d\n", numResult)
	return err
}
