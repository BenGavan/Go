...
type Parser interface {
	ParsePage(document *goquery.Document) ScrapeResult
}
...
func main() {
	d := DummyParser{}
	Crawl("https://en.wikipedia.org/wiki/Elon_Musk", d, 10)
	//out, err := getRequest("https://en.wikipedia.org/wiki/Elon_Musk")
	//handleError(err)
	//
	//doc, _ := goquery.NewDocumentFromResponse(out)
	//fmt.Println("\n\n", doc)
...
func extractLink(doc *goquery.Document) []string {
	foundUrls := []string{}
...
func crawlPage(baseURL, targetURL string, parser Parser, token chan struct{}) ([]string, ScrapeResult) {
	token <- struct{}{}
	incrementCounter()
	fmt.Println("Requesting: #" + strconv.Itoa(int(getCounter())), targetURL)
